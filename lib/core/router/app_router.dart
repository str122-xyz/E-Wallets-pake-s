...
class AppRouter {
  ...
  // Singleton -- harus instance YANG SAMA dipakai terus, karena listener
  // deep link (di main.dart) butuh memanggil router.go(...) dari luar
  // widget tree, jauh setelah app pertama kali dibangun.
  static GoRouter? _router;

  static GoRouter get router {
    if (_router == null) {
      debugPrint(
          '[DIAGNOSTIC] GoRouter instance baru dibuat (seharusnya cuma sekali!)');
    }
    return _router ??= GoRouter(
      ...
      routes: [
        ...
        GoRoute(
          ...
          builder: (_, state) {
            final extra = (state.extra as Map<String, dynamic>?) ?? {};
            return _withPayment(OtpVerifyPage(flowData: extra));
          },
        ),
        GoRoute(
          path: '/success',
        ...
        GoRoute(
          path: '/merchant',
          builder: (_, state) => MerchantCheckoutPage(
              deepLinkData: state.extra as Map<String, dynamic>?),
        ),
      ],
    );
  }
  ...
  static Widget _withAccount(Widget child) {
    return MultiBlocProvider(providers: [
      ...
      // begitu instance ini dibuat, supaya saldo tersedia tanpa perlu tiap
